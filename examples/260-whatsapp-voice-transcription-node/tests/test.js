...
// We can't run the full WhatsApp webhook server in CI (needs Meta webhook
// verification and real WhatsApp messages), but we CAN verify:
//   1. Deepgram API key works for STT (pre-recorded file transcription)
//   2. Express is installed and importable
...
function testExpressInstalled() {
  console.log('Testing express import...');
  const express = require('express');
  if (typeof express !== 'function') {
    throw new Error('express default export is not a function');
  }
  console.log('  express loaded successfully');
}

async function run() {
  testExpressInstalled();

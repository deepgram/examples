...
const TMP_WAV   = '/tmp/browser_test.wav';
...
function testStaticServing(port) {
  return new Promise((resolve, reject) => {
    http.get(`http://localhost:${port}/index.html`, (res) => {
      let body = '';
      res.on('data', c => (body += c));
      res.on('end', () => {
        if (res.statusCode !== 200) return reject(new Error(`/index.html returned ${res.statusCode}`));
        if (!body.includes('Deepgram Live Transcription'))
          return reject(new Error('index.html missing expected title'));
